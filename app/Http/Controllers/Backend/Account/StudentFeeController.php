...
public function add(){
 ...
 $access = Role::orderBy('id', 'ASC')->where('id', '=', Auth::user()->role_id)->first();
        if($access->accounts_management && $access_action->students_fee_act){
...
public function getstudent(Request $request){
    ...
    foreach ($data as $key =>$std){
        $student_fee=FeeCategoryAmount::where('fee_category_id',$fee_category_id)->where('class_id',$class_id)
            ->first();
        $accountstudentfees=AccountStudentFee::where('student_id',$std->student_id)->where('year_id',$std->year_id)->
            where('class_id',$std->class_id)->where('fee_category_id',$std->fee_category_id)->where('date',$date)->first();
...
;
        ...
        $html[$key]['tdsource'].='<td>' .'<input type ="hidden" name="student_id[]" value=" '.$std->student_id.'">'.'<input type="checkbox" 
...
if(!empty(@$data) ||empty ($checkdata)){
    ...
    return redirect()->route('accounts.fee.view')->with('success','Well done Successfully Update');
}else{
